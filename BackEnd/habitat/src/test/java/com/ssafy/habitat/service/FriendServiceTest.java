...
class FriendServiceTest {
    ...
    void getFriendList_ReturnFriendList() {
        ...
        when(friendRepository.findByMyId(user1)).thenReturn(userFriendList);
    ...
    @Test
    @DisplayName("친구 수 반환 테스")
    void getFriendCnt_ReturnUserFriendCount() {
        // Given
        User user1 = new User();
        int friendCnt = 5;
        when(friendRepository.countByMyId(user1)).thenReturn(friendCnt);

        // When & Then

        int result = friendService.getFriendCnt(user1);


        assertEquals(friendCnt, result);
    }
